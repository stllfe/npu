...
static bool is_valid_op(const std::string &op) {
  return op == "lshift" || op == "rshift";
}

static std::string pick_model(const std::string &op, int width) {
  std::string exact = "models/" + op + "_int32_1x" + std::to_string(width) + ".rknn";
  if (file_exists(exact)) return exact;
  std::string generic = "models/" + op + "_int32_1x1.rknn";
...
static void fill_inputs(int total, std::vector<int32_t> &input_x, std::vector<int32_t> &input_y) {
  input_x.resize(total);
  input_y.resize(total);
  for (int i = 0; i < total; i++) {
    input_x[i] = (i * 37) & 0x0000ffff;
    input_y[i] = i % 8;
  }
}

static int run_case(const std::string &op, int size) {
  if (size <= 0) return -1;
  int total = size * size;
  std::string model_path = pick_model(op, size);
  ...
  std::vector<int32_t> input_x;
  std::vector<int32_t> input_y;
  fill_inputs(total, input_x, input_y);
  ...
  if (op == "lshift") {
    for (int i = 0; i < total; i++) {
      uint16_t x = static_cast<uint16_t>(input_x[i]);
      uint16_t y = static_cast<uint16_t>(input_y[i]);
      expected[i] = static_cast<int32_t>(static_cast<uint16_t>(x << y));
    }
  } else {
    for (int i = 0; i < total; i++) {
      uint16_t x = static_cast<uint16_t>(input_x[i]);
  ...
  std::cout << "=== " << op << " " << size << "x" << size << " (" << model_path << ") ===" << std::endl;
...
static bool parse_size(const char *text, int &size_out) {
  if (!text || !*text) return false;
  for (const char *p = text; *p; ++p) {
    if (*p < '0' || *p > '9') return false;
  }
  size_out = std::atoi(text);
  return size_out > 0;
}
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    int status = 0;
    for (const char *op : {"lshift", "rshift"}) {
      for (int size : sizes) {
        int ret = run_case(op, size);
        if (ret == 2) {
          std::cout << "=== " << op << " " << size << "x" << size << " ===" << std::endl;
          std::cout << "SKIP: missing model " << op << "_int32_1x" << size << ".rknn (and " << op
                    << "_int32_1x1.rknn)" << std::endl;
          continue;
        }
        if (ret != 0) status = ret;
      }
  ...
  if (argc == 2) {
    ...
    if (!is_valid_op(op)) {
      ...
      std::cerr << "usage: " << argv[0] << " <lshift|rshift> [size]" << std::endl;
      return -1;
    }
