...
- Table: 1026 entries covering [X_MIN, X_MAX] (default [-5, 0]).
...
def _decode_fp16(raw: int) -> float:
    return float(np.array(raw, dtype=np.uint16).view(np.float16)[0])
...
if __name__ == "__main__":
    ...
    atol = 1e-6
    rtol = 1e-3
    passes = np.all(diffs <= (atol + rtol * np.abs(expected)))
